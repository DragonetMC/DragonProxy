...
public class PCSpawnPlayerPacketTranslator implements IPCPacketTranslator<ServerSpawnPlayerPacket> {
	...
	public PEPacket[] translate(UpstreamSession session, ServerSpawnPlayerPacket packet) {
		try {
			...
			pkAddPlayer.position = new Vector3F((float) packet.getX(), (float) packet.getY() + 1.62f,
					(float) packet.getZ());
			...
			/*
			 * PlayerListEntry entry = new PlayerListEntry(); entry.uuid = packet.getUUID();
			 * entry.eid = pkAddPlayer.eid; entry.username = pkAddPlayer.username;
			 * entry.skin = Skin.defaultSkin; entry.xboxUserId = ""; PlayerListPacket lst =
