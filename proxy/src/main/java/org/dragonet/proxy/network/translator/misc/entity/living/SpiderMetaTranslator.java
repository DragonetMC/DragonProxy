...
public class SpiderMetaTranslator extends AbstractInsentientMetaTranslator {

    @Override
    public void translateToBedrock(ProxySession session, EntityDataMap dictionary, EntityMetadata metadata) {
        if(metadata.getId() == 15) {
            // 0x01 is climbing
            dictionary.getFlags().setFlag(EntityFlag.WALL_CLIMBING, ((byte) metadata.getValue() & 0x01) > 0);
        }
