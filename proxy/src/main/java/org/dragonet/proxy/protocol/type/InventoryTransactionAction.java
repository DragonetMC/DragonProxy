...
public class InventoryTransactionAction {
	 ...
	 *
	 * These identifiers are used for inventory source types which are not currently
	 * implemented server-side in MCPE. As a general rule of thumb, anything that
	 * doesn't have a permanent inventory is client-side. These types are to allow
	 * servers to track what is going on in client-side windows.
	 *
