...
public class PlayerList extends Packet {
	...
	public class Add extends Packet {
		...
		@Override
		public int length() {
			int length=Buffer.varuintLength(players.length) + 0; for(sul.protocol.pocket101.types.PlayerList cgxhewvycw:players){ length+=cgxhewvycw.length(); } return length;
		}
		...
		public byte[] encode() {
			...
			this.writeVaruint((int)players.length); for(sul.protocol.pocket101.types.PlayerList cgxhewvycw:players){ this.writeBytes(cgxhewvycw.encode()); }
			return this.getBuffer();
		}
		...
		public void decode(byte[] buffer) {
			this._buffer = buffer;
			int bhbsyxllcnm=this.readVaruint(); players=new sul.protocol.pocket101.types.PlayerList[bhbsyxllcnm]; for(int cgxhewvycw=0;cgxhewvycw<players.length;cgxhewvycw++){ players[cgxhewvycw]=new sul.protocol.pocket101.types.PlayerList(); players[cgxhewvycw]._index=this._index; players[cgxhewvycw].decode(this._buffer); this._index=players[cgxhewvycw]._index; }
		}
